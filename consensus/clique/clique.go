		...
		*/
		...
		if c.reputation > 0 {
			for i := 0; i < len(snap.TallyStakes); i++ {
				if snap.TallyStakes[i].Owner == max_staked_address {
					index = i
					break
				}
			}
			...
			if index != -1 {
				snap.TallyStakes[index].Reputation = c.reputation
			}
		}
	}
...
func (c *Clique) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	*/
	...
	if c.malicious == true {
		...
		fmt.Println("before", c.reputation)
		c.stake = c.stake - (c.stake * 1 / 4)
		c.reputation = c.reputation - 2
		fmt.Println("Downgrading This Node")
		fmt.Println("After", c.stake)
		fmt.Println("After", c.reputation)
		c.malicious = false
